...
export async function GET(request: NextRequest) {
  try {
    ...
    const { revenueData } = generateFilteredData(dateRange as DateRange, userType as UserType);

    // Add security headers
    const response = NextResponse.json(revenueData);
    response.headers.set('X-Content-Type-Options', 'nosniff');
    response.headers.set('X-Frame-Options', 'DENY');
    
    return response;
  } catch (error) {
    console.error('API Error in /api/revenue:', error);
    return NextResponse.json(
